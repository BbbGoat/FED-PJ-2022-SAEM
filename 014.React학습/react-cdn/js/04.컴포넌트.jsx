...
/**************************************************** 
    ...
    3. 컴포넌트는 다음 2가지 생성가능함
        1) 클래스형 컴포넌트
        2) 함수형 컴포넌트
        (-> 우리는 함수형 컴포넌트에 집중할 예정!)

    -> 클래스형 컴포넌트는 리액트 초중기에 주로
        사용되었으나... React 16.8 버전에서 Hooks와
        함께는 더 이상 사용되지 않는다!
        Hooks는 함수형 컴포넌트에서만 사용 가능하다!

    ___________________________________________

    [ 첫번째 컴포넌트 만들기! ]
    - 리액트 컴포넌트 이름은 반드시 첫글자를 대문자로 만든다!
    (안지키면 적용안됨!!!)
...
// 함수 컴포넌트에서는 표현식 안에서 {props.호출시사용한속성명}
// 여기서는 {props.color} 이런 형식으로 사용함!

// 컴포넌트 재사용하기!!!
ReactDOM.render(<Favorite color="파란색" food="알리오올리오" hobby="여행" />, document.querySelector("#root4"));
