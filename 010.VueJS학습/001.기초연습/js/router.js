...
/*************************************************** 
    ...
    3. 설치 : 
        ...
        (2) Vue CLI 방식 : npm 설치
            ...
            -> 설치후 import로 사용할 페이지에 팩키지를 포함시킨다!
...
let Foods = {
    template: `<div v-bind:class="'foods router '+this.$route.params.cls">World Foods {{ this.$route.params.item }}</div>`
};
...
const router = new VueRouter({
    routes: [
        ...
        // 세번째 루트의 파라미터 버전 루트추가!
        {
            // 파라미터를 받는 같은 path의 루트는
            // 호출과 구분을 위해 반드시 name속성을 설정하여야 함!
            name: '/umsik',
            path: "/foods:item", // 경로뒤에 콜론(:)을쓰고 뒤에 파라미터 변수를 써준다!
            component: Foods,
        }
    ],
});
